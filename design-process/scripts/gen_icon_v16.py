...
GLOW = (34, 52, 92)           # 골드 노브 쪽으로 은은하게 밝아지는 새벽빛 톤 (NAVY보다 살짝 밝고 따뜻함)


def make_gradient_bg(s, radius_ratio, center_ratio, c_near, c_far, max_ratio=1.0):
    """center_ratio 위치(골드 노브)를 중심으로 c_near -> c_far 로 퍼지는 radial gradient 배경."""
...
def render_icon(s, glow=False, radius_ratio=0.18):
    if glow:
        # 골드 노브 위치(=0.74, 0.54) 쪽으로 은은하게 밝아지는 '새벽빛' 그라디언트
...
CASES = [
    ("G-1", "기준 (단색 배경)", "v13b 기존안",
     lambda s: render_icon(s, glow=False)),
    ("J-1", "Dawn Glow 배경", "골드 노브 쪽 은은한 새벽빛 그라디언트",
     lambda s: render_icon(s, glow=True)),
]
...
img.save("/sessions/elegant-fervent-mendel/mnt/outputs/WeatherON_v16_test.png")
print("done", img.size)
...
img2.save("/sessions/elegant-fervent-mendel/mnt/outputs/WeatherON_v16_favicon_test.png")
