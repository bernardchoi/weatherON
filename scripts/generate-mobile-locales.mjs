...
const overrides = {
  en: {
    ...
    "현재 위치 좌표는 날씨·장소·경로 조회 시 WeatherON의 Cloudflare Worker를 거쳐 기상청·Apple WeatherKit·Open-Meteo·Kakao·Google 등 해당 기능 제공자에 전달됨. D1·R2에는 저장하지 않으며 Worker 요청 로그의 쿼리 문자열은 가림 처리": "Current-location coordinates are sent through WeatherON's Cloudflare Worker to the provider needed for the requested weather, place, or route lookup, such as the Korea Meteorological Administration, Apple WeatherKit, Open-Meteo, Kakao, or Google. They are not stored in D1 or R2, and query strings are redacted from Worker request logs",
    ...
    "보유기간: 계정 정보·동의 기록·서버 저장 데이터는 계정 삭제 시까지, 기기 저장 데이터는 앱에서 삭제하거나 앱을 제거할 때까지 보관. 법령상 별도 보관 의무가 확인된 정보는 해당 기간 분리 보관": "Retention: Account information, consent records, and server-stored data are retained until the account is deleted. Data stored on the device is retained until it is deleted in the app or the app is removed. Information subject to a confirmed statutory retention duty is segregated and retained for the required period",
    "파기 절차·방법: 계정 삭제 요청이 완료되면 운영 데이터베이스의 계정 행과 연결 데이터를 삭제하고, 기기 데이터베이스 기록과 옷장 사진 파일을 삭제. 복구용 사본의 보존·삭제 주기는 인프라 설정 확인 후 공개본에 명시 필요": "Deletion procedure and method: When an account-deletion request is completed, the account row and linked data are deleted from the production database, and records in the device database and wardrobe-photo files are deleted. The retention and deletion schedule for recovery copies must be stated in the public version after the infrastructure settings are confirmed",
    "처리위탁: Cloudflare Workers·D1·Workers AI를 인증, 서버 기능, 계정 데이터 저장, 옷장 사진 분석에 사용. 실제 계약상 수탁관계, 처리 국가와 보존기간은 확인 전이므로 공개본 확정 필요": "Processing service providers: Cloudflare Workers, D1, and Workers AI are used for authentication, server functions, account-data storage, and wardrobe-photo analysis. The public version cannot be finalized until the contractual processor relationship, processing countries, and retention periods are confirmed",
    "외부 제공: 로그인 제공자와 날씨·장소·경로 제공자에는 이용자가 요청한 인증 또는 조회에 필요한 토큰, 검색어, 좌표를 전송할 수 있음. 업체별 제3자 제공·위탁 구분과 국외이전 항목은 운영 구성 확인 후 공개본 확정 필요": "External disclosures: Tokens, search terms, and coordinates needed for authentication or a lookup requested by the user may be sent to sign-in, weather, place, and route providers. The public version must be finalized after confirming the production configuration, including whether each recipient is a third party or processor and the applicable cross-border transfer details",
    ...
    "사업자 법적 명칭·주소·전화번호와 Cloudflare 처리지역·위탁관계가 확인되지 않아 해당 정보는 임의 기재하지 않았으며, 외부 공개 전 반드시 보완 필요": "The operator's legal name, address, and telephone number, and Cloudflare's processing locations and processor relationship have not been confirmed. These details have not been invented and must be completed before external publication",
    "이용약관": "Terms of Use",
  ...
  ja: {
    ...
    "현재 위치 좌표는 날씨·장소·경로 조회 시 WeatherON의 Cloudflare Worker를 거쳐 기상청·Apple WeatherKit·Open-Meteo·Kakao·Google 등 해당 기능 제공자에 전달됨. D1·R2에는 저장하지 않으며 Worker 요청 로그의 쿼리 문자열은 가림 처리": "現在地の座標は、天気・場所・経路の照会時にWeatherONのCloudflare Workerを経由し、韓国気象庁、Apple WeatherKit、Open-Meteo、Kakao、Googleなど、その機能に必要な提供事業者へ送信されます。D1またはR2には保存せず、Workerのリクエストログではクエリ文字列をマスキングします",
    ...
    "보유기간: 계정 정보·동의 기록·서버 저장 데이터는 계정 삭제 시까지, 기기 저장 데이터는 앱에서 삭제하거나 앱을 제거할 때까지 보관. 법령상 별도 보관 의무가 확인된 정보는 해당 기간 분리 보관": "保存期間：アカウント情報、同意記録およびサーバー保存データはアカウント削除まで、端末保存データはアプリ内で削除するかアプリを削除するまで保存します。法令上の保存義務が確認された情報は、所定の期間、分別して保存します",
    "파기 절차·방법: 계정 삭제 요청이 완료되면 운영 데이터베이스의 계정 행과 연결 데이터를 삭제하고, 기기 데이터베이스 기록과 옷장 사진 파일을 삭제. 복구용 사본의 보존·삭제 주기는 인프라 설정 확인 후 공개본에 명시 필요": "削除の手続および方法：アカウント削除の申出が完了すると、本番データベースのアカウント行および関連データを削除し、端末データベースの記録およびワードローブ写真ファイルを削除します。復旧用コピーの保存・削除期間は、インフラ設定の確認後に公開版へ記載する必要があります",
    "처리위탁: Cloudflare Workers·D1·Workers AI를 인증, 서버 기능, 계정 데이터 저장, 옷장 사진 분석에 사용. 실제 계약상 수탁관계, 처리 국가와 보존기간은 확인 전이므로 공개본 확정 필요": "取扱いの委託：認証、サーバー機能、アカウントデータの保存およびワードローブ写真の分析にCloudflare Workers、D1およびWorkers AIを使用します。契約上の委託関係、取扱国および保存期間を確認するまで、公開版を確定できません",
    "외부 제공: 로그인 제공자와 날씨·장소·경로 제공자에는 이용자가 요청한 인증 또는 조회에 필요한 토큰, 검색어, 좌표를 전송할 수 있음. 업체별 제3자 제공·위탁 구분과 국외이전 항목은 운영 구성 확인 후 공개본 확정 필요": "外部提供：利用者が求めた認証または照会に必要なトークン、検索語および座標を、ログイン、天気、場所および経路の提供事業者へ送信する場合があります。各事業者について第三者提供か委託かの区分および外国移転事項を本番構成で確認した後、公開版を確定する必要があります",
    ...
    "사업자 법적 명칭·주소·전화번호와 Cloudflare 처리지역·위탁관계가 확인되지 않아 해당 정보는 임의 기재하지 않았으며, 외부 공개 전 반드시 보완 필요": "事業者の法的名称、住所および電話番号ならびにCloudflareの取扱地域および委託関係は未確認です。これらを推測で記載しておらず、外部公開前に必ず補完する必要があります",
    "이용약관": "利用規約",
