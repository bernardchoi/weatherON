...
const adbStatusDoc = readOptionalText(adbStatusPath);
const installStatusPath = join(rootDir, "docs/architecture/WeatherON_ANDROID_INSTALL_STATUS.md");
const installStatusDoc = readOptionalText(installStatusPath);
...
const checks = [
  ...
  check("Preview preflight 명령", Boolean(packageConfig.scripts?.["check:android-preview-preflight"]), "npm run check:android-preview-preflight"),
  check("핵심 클릭 흐름 체크 명령", Boolean(packageConfig.scripts?.["check:android-core-flow"]), "npm run check:android-core-flow"),
  ...
  check("Preview APK 소스 일치", previewBuildMatchesSource, `${latestPreviewBuildVersion} vs ${sourceVersion}`),
...
| 최신 preview build version | \`${latestPreviewBuildVersion}\` |
...
function currentRuntimeBlocker() {
  ...
  if (isQuotaBlocked) return "EAS Android build quota reset 대기";
  if (!previewBuildMatchesSource) return "최신 소스 반영 preview APK 필요";
