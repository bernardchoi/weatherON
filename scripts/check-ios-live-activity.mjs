...
assert.match(nativeModule, /recoverLongExpiredDepartureActivities/u);
assert.match(nativeModule, /activityState == \.stale/u);
assert.match(nativeModule, /activityState == \.pending/u);
assert.match(nativeModule, /if #available\(iOS 26\.0, \*\), startAt > Date\(\)/u);
assert.match(nativeModule, /alertConfiguration: alert/u);
assert.match(nativeModule, /await activity\.update\(content\)/u);
assert.match(nativeModule, /endOtherDepartureActivities\(keeping: activity\.id\)/u);
...
assert.match(liveActivity, /WeatherONDepartureExpandedLabel\(\)\s*\.padding\(\.leading, 18\)/u);
assert.match(liveActivity, /style: \.expanded\s*\)\s*\.padding\(\.trailing, 18\)/u);
assert.match(liveActivity, /departureTimeLabel: context\.state\.departureTimeLabel \?\? context\.attributes\.departureTimeLabel\s*\)\s*\.padding\(\.horizontal, 18\)/u);
...
assert.match(liveActivity, /context\.isStale/u);
assert.match(liveActivity, /context\.state\.guidanceSymbol/u);
assert.doesNotMatch(liveActivity, /systemName: "cloud\.rain\.fill"/u);
...
assert.match(liveActivityProvider, /automaticSyncQueue/u);
assert.match(liveActivityProvider, /scheduleRegistrationRetry/u);
...
assert.match(liveActivityShared, /우산 챙겨요/u);
assert.match(liveActivityShared, /가볍게 출발해요/u);
