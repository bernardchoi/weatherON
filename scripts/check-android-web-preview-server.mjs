...
function checkBottomNavBundle(sources) {
  ...
  const stale = ['label:"우산"', 'label:"\\uc6b0\\uc0b0"', 'label:"강수"', 'label:"\\uac15\\uc218"'];
  ...
  if (missing.length > 0) {
    issues.push("dist bundle does not prove bottom nav labels 홈/코디/출발/MY/소셜");
  }
  if (staleFound.length > 0) {
    issues.push(`dist bundle contains stale bottom nav labels: ${staleFound.join(", ")}`);
  }
  return missing.length === 0 && staleFound.length === 0 ? "홈/코디/출발/MY/소셜 확인" : "확인 필요";
}
