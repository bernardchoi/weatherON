...
> **2026-09-12 제품화 해석 갱신:** 이 문서는 바람의 탑에서 출발한 범용 연구 이력임. WeatherON 적용은 [전용 디자인 방향](perfora_air_v1_1_experimental_addon/docs/01_experimental_direction_brief.md)을 우선함. ‘분위기 우선’은 정보 지연을 뜻하지 않으며, 일정/실내 공기를 종합한 압력·움직임·환기 예시는 WeatherON 제품 규칙으로 채택하지 않음. 원전의 환경 반응 원리와 Quiet Horizon을 연결하고 홈·날씨 상세·출발로 검증 범위를 좁힘.
