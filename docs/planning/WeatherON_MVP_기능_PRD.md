...
- 코디, 우산, 신발 추천은 정식 출시 후보에 포함되는 준비 판단 기능으로 둔다.
- Android 비공개 테스트 14일 운영 안에 코디 추천, 코디 상세, 저장 gate, 옷장 프리셋 진입을 검증한다.
...
| 코디 | C1/C2/C3/C4 | 코디 추천, 옷장, 옷 등록, 코디 상세. MVP1/Beta/v1.0 포함 기능 |
| 출발/목적지 | G1/G2/P1/P2/P3 | 목적지 목록, 목적지 케어, 목적지 추가, 준비 가이드, 필터/상세 |
| 설정/정책 | M1/M2/M3/R1/R2/R3/R4 | MY, 알림 설정, 전역 설정, 정책/광고 |

### 현재 MVP 기능 검증 우선순위

2026-07-08 현재 구현 검증은 `H1 홈`, `C1~C4 코디`, `G1 출발`, `M1 MY`를 우선한다.
...
| C1~C4 코디 | 날씨 근거형 코디 추천, 옷장 프리셋, 저장 gate, 상세 | H1, 하단 코디 탭, A2/A3 |
...
도보여행 세부 기능은 `WeatherON_도보여행_PRD.md`, 여행 메뉴·무료/프리미엄·권한 경계는 `WeatherON_기능_출시_로드맵.md`의 여행 공통 기준을 따른다. 향후 `출발 → 여행` 안에 도시 여행·도보 코스를 구성하며, MVP 사용 지표 확인 전까지 출시 범위에서 제외한다.
...
- 기존 이동수단 선택은 카드 4개 나열이 아니라 선택 시트 안의 리스트로 제공하며, 수단 선택 즉시 리스트를 닫고 선택값을 반영한다. 향후 경로 조회 단계에도 같은 화면에서 기본 결과를 표시한다. 다른 경로를 원할 때만 선택 시트에서 비교·적용한다.
- 같은 시간 시트에서 `도착 시간` 또는 `출발 시간` 기준을 하나 선택하고 `시/분` 스크롤로 시각을 고른다. 분 선택은 5분 단위로 제공한다.
- 도착 기준은 이동시간과 준비 여유를 역산해 출발 권장을 표시한다. 출발 기준은 선택 시각을 유지하고 이동시간이 있을 때만 예상 도착을 표시한다.
...
### F6-1. 일상 이동 경로 확장 — 화면 기획, MVP 자동 포함 아님

[일상 이동 경로 화면 기획](WeatherON_일상_이동_경로_화면_기획.md)에 G2 기본 요약·국내 실제 경로 후보·실패/복귀 기준을 정의한다. 출시 범위는 로드맵의 일상 이동 확장 기준을 따른다.

- 국내 카카오맵 REST 상세 응답의 실제 후보·노선·환승·요금·도보 단계를 앱 안에서 제공한다. 해외 상세 경로는 Google Routes 승인 전 보류하고 Google Maps 외부 연결을 보조 수단으로 둔다. 신규 공급자는 임의 도입하지 않는다.
- G1 요약 카드와 기존 G2 도착·이동 칩을 재사용한다. 경로 비교는 선택 시트, 이동 날씨는 G2의 자세히 펼침으로 제공한다. 별도 전체 화면이나 여행 라우트를 추가하지 않는다. 최초 추천 미리보기는 저장·알림 설정을 실행하지 않으며 유효한 기존 선택을 우선한다.
- 기본 추천은 G2 미리보기다. Guest/미저장 목적지의 경로 적용은 현재 화면에만 반영한다. 계정 연결된 저장 목적지의 적용은 저장·G1 계산을 갱신하고, 이미 켜진 지원 일정의 알림만 교체 요청한다. 저장·예약 실패는 각각 표시하며 기본 추천만으로 저장이나 알림을 시작하지 않는다. 반복 알림은 앱 미실행 시 다음 일정 경로 재검증까지 검증한 뒤 활성화한다.
- Guest 조회·현재 선택 적용·외부 길찾기는 허용한다. 지속 저장 시 계정 연결, 알림 신청 시 계정·알림 권한이 필요하다.
- 도착 목표 검색 지원 여부, 조회 결과 유효기간과 저장 약관은 공급자 검증 후 확정한다. 기존 DestinationCare 타입은 구현 시 확장하며 현재 계약에 미구현 필드를 추가하지 않는다.
...
- Android 비공개 테스트 14일 운영 안에 코디 포함 build로 전환
...
- M1/M3 설정 허브 정리
- 라이트/다크 접근성 QA
- Play 비공개 테스트, 스토어 스크린샷, 개인정보/약관 최종화
- 코디 추천/상세/저장 gate/옷장 프리셋 실기기 QA
- 코디 개인화·AI 재추천 고도화 여부는 MVP 1 반응 확인 후 결정
...
| 날짜 | 내용 |
|---|---|
| 2026-09-08 | F6-1 일상 이동 경로 화면 기획 연결 및 단계별 적용/권한 경계 추가 |
