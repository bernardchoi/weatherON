...
WeatherON MVP의 핵심 목표는 **오늘 준비 5초 완결**이다.
사용자는 앱을 열고 현재 위치 또는 목적지 기준으로 오늘 입을 옷, 우산 필요 여부, 신발, 출발 준비 알림을 빠르게 판단해야 한다.
...
- Guest도 날씨, 코디, 우산, 목적지 미리보기를 바로 경험한다.
...
| 지표 | MVP 목표 |
|---|---|
| 첫 실행 후 홈 도달 | 스플래시 후 H1 Guest 홈 진입 |
| 홈 의사결정 | 현재 날씨, 코디, 우산/비 신호, 목적지 추천을 5초 내 파악 |
| 계정 gate | 저장/동기화/알림 확장 액션에서만 A2/A3 호출 |
| 추천 안정성 | 날씨 API 실패 시 캐시/기본 위치/수동 위치로 빈 화면 방지 |
| 알림 구조 | H1 실시간 카드 확인 후 숨김, H3 이력 유지 |
...
| 온보딩 | O2/O3/O4/O5/O6 | 기능 소개, 권한, 스타일, 알림 기준, 목적지 등록 |
| 홈/날씨 | H1/H2/H3/H4/H5 | 홈, 위치 변경, 알림 센터, 우산 추천, 강수 타임라인 |
| 코디 | C1/C2/C3/C4 | 코디 추천, 옷장, 옷 등록, 코디 상세 |
...
도보여행은 `WeatherON_도보여행_PRD.md`를 별도 기준으로 유지한다.
...
- M2는 모든 알림을 세세히 나열하기보다 현재 자동 케어 상태와 고급 설정만 제공한다.
...
2. H1에서 코디/우산/목적지 추천 확인
3. 저장 액션 탭 시 A2/A3 계정 연결
4. A3 완료 후 원래 화면과 액션 상태로 복귀
5. O3 권한 완료 후 알림/목적지 케어 상태 복귀
6. H4/H5/G2에서 M2 진입 후 원래 추천 화면으로 복귀
...
*WeatherON MVP 기능 PRD v1.0 · June 2026*
