...
WeatherON MVP의 핵심 목표는 **나갈 시간과 비 타이밍을 5초 안에 판단**하게 하는 것이다.
사용자는 앱을 열고 목적지 기준 출발시간, 현재 위치와 목적지 날씨 차이, 비 시작/그침 시간을 빠르게 판단해야 한다.
...
- 코디, 우산, 신발 추천은 핵심 출발/강수 판단을 돕는 보조 기능으로 둔다.
...
| 홈 의사결정 | 출발 추천 시간, 목적지 날씨 차이, 강수 타임라인 진입을 5초 내 파악 |
...
| 온보딩 | O2/O3/O4/O5/O6 | 출발/강수 중심 기능 소개, 권한, 스타일, 알림 기준, 목적지 등록 |
