...
- 하단 메뉴 최종 기준은 `홈 / 출발 / MY`이다. 코디·옷장은 `MY` 안의 `코디·옷장`에서 진입한다. `코디`, `소셜`, `우산`, `강수`가 하단에 보이면 구버전 preview/APK 또는 mockup preview를 보고 있는 상태로 판정한다.
...
- 홈 판단 CTA와 코디·옷장 클릭 흐름은 `npm run check:android-core-flow`로 반복 확인한다.
...
| 핵심 클릭 흐름 | `npm run check:android-core-flow`로 홈 3개 CTA, MY 코디·옷장, 기준 저장, 옷장 추가 반영 확인 |
...
| 하단 탭 IA | QA용 `코디/소셜/우산/강수` 직접 탭이 MVP 공개 범위와 달랐음 | `홈/출발/MY + MY 코디·옷장`로 고정 |
...
3. `MY -> 코디·옷장` 진입 후 기준 저장과 옷장 추가 반영 확인
...
| 2026-06-28 | In-app browser 390x844에서 하단 탭 `홈/출발/MY + MY 코디·옷장` 확인 |
| 2026-06-30 | 소셜 레이어 최초 출시 미공개 기준으로 web export QA 기준 갱신 |
