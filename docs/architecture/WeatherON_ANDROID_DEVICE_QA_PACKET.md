...
| D2 | 첫 실행 | 크래시 없이 온보딩 진입 | 통과 | qa-local APK 1.0.0 (10) r2 COLD launch `TotalTime=224ms`, MainActivity foreground, crash buffer 0 lines |
...
| D4-2 | 핵심 클릭 흐름 | `npm run check:android-core-flow` 기준 홈 CTA, 코디 탭 추천/상세/저장/옷장 진입, 하단 탭 가림 없음 | 통과 | 2026-07-20 r2에서 코디 목록 -> 상세 -> 저장 완료 확인. 하단 CTA/탭 가림 없음 |
...
| D8 | 위치 권한 거부 | 앱 사용 가능, 수동 위치/목적지 흐름 유지 | 통과 | r2에서 ACCESS_FINE_LOCATION/COARSE_LOCATION revoke 후 홈 유지. crash buffer 0 lines. 테스트 후 권한 복구 |
| D9 | 목적지 검색 | 1순위 국내 장소(Kakao Local 또는 fallback) 검색·저장 확인 후 2순위 해외 장소(Google 또는 fallback) 보조 확인 | 통과 | r2에서 `Jamsil` 검색 결과 `잠실야구장` 선택/저장/상세 확인. 테스트 목적지 삭제 완료 |
| D10 | 화면 밀도 | 작은 화면에서 가로 overflow/버튼 잘림/하단 탭 CTA 가림 없음 | 통과 | 1084x2412 실기기 화면에서 홈/코디/출발/MY/알림 설정 주요 카드와 하단 탭 가림 없음 |
| D11 | 다크/라이트 | 텍스트 대비와 버튼 상태 정상 | 통과 | r2에서 `cmd uimode night yes` 홈 확인, crash buffer 0 lines. 테스트 후 light mode 복구 |
| D12 | 네트워크 끊김 | 빈 화면 없이 최근/기본 예보 안내 표시 | 통과 | r2에서 Wi-Fi/data off 후 active default network none. 홈 빈 화면 없음, `최근 예보로 유지 중` 표시. 테스트 후 Wi-Fi/data 복구 |
