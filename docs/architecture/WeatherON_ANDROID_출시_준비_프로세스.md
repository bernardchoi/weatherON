...
- 하단 탭은 MVP 기준 `홈 / 출발 / MY`로 고정한다. 코디·옷장은 `MY > 코디·옷장`에서 진입하고, 소셜/우산/강수는 하단 직접 탭으로 공개하지 않는다.
...
2. 하단 탭 `홈/출발/MY` 구성과 `코디/소셜/우산/강수` 직접 탭 제거 확인
...
| 2026-06-30 | 하단 탭 IA를 MVP 기준 `홈/출발/MY`로 고정하고 코디·옷장은 MY 내부 진입으로 정리 |
