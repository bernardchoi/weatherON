...
- 하단 탭은 목업 기준 `홈 / 코디 / 출발 / MY / 소셜`로 고정한다. `우산`과 `강수`는 하단 직접 탭이 아니라 홈/알림 흐름 안에서 진입한다.
...
2. 하단 탭 `홈/코디/출발/MY/소셜` 구성과 `우산/강수` 직접 탭 제거 확인
...
| 2026-06-28 | 하단 탭 IA를 목업 기준 `홈/코디/출발/MY/소셜`로 고정 |
| 2026-06-28 | 하단 `소셜`을 `S1 ON Square` 화면으로 분리하고 알림센터 임시 연결 제거 |
