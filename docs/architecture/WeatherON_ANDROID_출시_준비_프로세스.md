...
| EAS 로그인 상태 | `npm run check:eas-login-state` 승격 실행 결과 `Not logged in` |
...
| A2 | Expo 계정/EAS 프로젝트 연결 | EAS project id | 로그인 필요 |
...
| C5 | EAS credentials 설정 | Android keystore | 미완료 |
...
| D3 | Google Maps Geocoding 키 발급 여부 결정 | 해외 목적지 실제 연동 시 |
...
| 순서 | 검증 | 도구 | 상태 |
|---|---|---|---|
| E1 | TypeScript 빌드 검증 | `npx tsc -p apps/mobile/tsconfig.json --noEmit` | 진행 필요 |
| E2 | shared rule 검증 | `npm run check:shared` | 진행 필요 |
| E3 | mockup 빌드 검증 | `npm run build:mockups` | 진행 필요 |
...
| E6 | EAS 로그인 상태 확인 | `npm run check:eas-login-state` | 로그인 필요 |
...
| E10 | Android APK 설치/실행 | 실제 기기 또는 에뮬레이터 | 진행 필요 |
...
| 앱 카테고리 | 날씨 또는 라이프스타일 검토 | 미완료 |
| 연락처 | 개발자 이메일/웹사이트 | 미완료 |
| 데이터 보안 | 위치, 계정, 알림, 광고 데이터 선언 | 초안 완료 |
| 콘텐츠 등급 | 설문 완료 | 미완료 |
| 광고 포함 여부 | AdMob 도입 시 예 | 미완료 |
...
1. `npm run eas:login`으로 Expo 계정 로그인
2. `npm run check:eas-login-state`로 로그인 상태 확인
3. `npm run eas:init`으로 EAS project id 연결
4. `npm run check:android-local-release-ready`로 스토어 자산/리포트/로컬 통합 게이트 갱신
5. `npm run check:android-build-ready` 필요 시 단독 재확인
6. `npm run build:android:preview`로 Android preview APK 빌드 실행
7. 실제 Android 기기 또는 에뮬레이터 QA 준비
8. Play Console 계정 유형 확인
9. `npm run check:android-store-submit-ready`로 Play 제출 blocker 확인
10. 스토어 등록 자료 미해결 항목 확정
11. `npm run build:android:production` 실행 전 EAS credentials 확인
