...
| 항목 | 상태 | 증거 |
| --- | --- | --- |
| 실제 기기 설치 | 통과 | `000841458003652`, `com.weatheron.mobile/.MainActivity` |
...
| H7 내일 브리핑 클릭 딥링크 | 통과 | 2026-07-13 17:08 KST 실제 테스트 푸시 탭 후 H7 `내일 브리핑` 이동 |
...
| Activity | `com.weatheron.mobile/.MainActivity` |
| version | `0.1.0 (6)` |
...
```text
0|com.weatheron.mobile|0|weatheron:test:1783164105758|10299
0|com.weatheron.mobile|0|weatheron:smart:rain-1h|10299
```
...
```text
adb -s 000841458003652 shell pm revoke com.weatheron.mobile android.permission.POST_NOTIFICATIONS
```
...
```text
adb -s 000841458003652 shell pm grant com.weatheron.mobile android.permission.POST_NOTIFICATIONS
```
...
```text
0|com.weatheron.mobile|0|weatheron:test:1783164705089|10299
```
...
0|com.weatheron.mobile|0|weatheron:test:H3:1783167761610|10299
```
...
0|com.weatheron.mobile|0|weatheron:test:H5:1783167583026|10299
```
...
```text
0|com.weatheron.mobile|0|weatheron:smart:destination-change:google-ChIJf6ZNmBDnAGARzJXusFHTcn4|10299
```
...
## 7. 2026-07-13 H7 내일 브리핑 실기기 QA

| 항목 | 결과 |
| --- | --- |
| 기기 | `000841458003652`, Android 16 |
| 설치 빌드 | `0.1.0 (8)` |
| APK sha256 | `613165711752ed972858b8537c0aed1cc68ce567fe10f6ac814b2e1c35a76c8f` |
| 알림 권한 | `POST_NOTIFICATIONS: granted=true` |
| 발송 경로 | M2 고급 설정의 `내일 브리핑 확인 알림 보내기` |
| 실제 수신 | 알림 셰이드 제목 `내일 브리핑`, 본문 `알림을 누르면 내일 날씨와 코디를 확인함` 확인 |
| 탭 결과 | H7 `내일 브리핑`으로 이동, 홈 탭 활성 상태 유지 |

탭 후 확인된 화면 문구:

```text
내일 브리핑
7월 14일 (화) · 고양시 덕양구 지축동
내일 날씨
최고 30° · 최저 25°
강수 확률 60%
내일 코디
내일 최대 강수확률 60% 기준으로 비 대응 아이템 우선
우산 준비
```

결론:

- 자기 전 알림의 전용 도착 화면 H7 동작 통과.
- 다음날 일별 예보 기반 날씨·코디·우산 정보와 `오늘` 기준 문구 미노출 확인.
