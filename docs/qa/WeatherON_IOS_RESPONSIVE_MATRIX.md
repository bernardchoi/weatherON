...
| P0 | 홈·날씨 | H1, H2, H3, H4, H5, H6, H7 | 카드 밀도, 사이드 패널, 차트 | 통과 | 부분 | 통과 | 통과 | 부분 |
| P0 | 코디·옷장 | C1, C2, C3, C4 | 2열 카드, 이미지 비율, 상세 CTA | 통과 | 부분 | 통과 | 통과 | 부분 |
| P0 | 목적지 | G1, G2, P1 | 검색 결과, 시간 선택, 고정 CTA | 부분 | 부분 | 부분 | 부분 | 대기 |
| P0 | MY·설정 | M1, M2, M3, M4 | 설정 행, 스위치, 하단 내비게이션 | 부분 | 부분 | 부분 | 부분 | 대기 |
...
| 홈·날씨 H1~H7 | H1은 기기 높이에 따라 compact/regular 요약 밀도를 분리하고 특보 문구·크기를 소형 화면에 맞춤. short 히어로·날씨 패널·차트 축소, regular/tablet 최대 폭 제한, H7 세로 스크롤, 44pt 조작부 유지 |
...
| 목적지 G1·G2·P1 | SE3 카드 하단 스택, G2 시간 요약 2+1·요일 칩, P1 검색 결과·고정 CTA 최대 폭과 여백 조정 |
| MY·설정 M1~M4 | 프로필·상태 카드, 알림 히어로, 표시 세그먼트, 권한 2버튼 행의 높이·여백·최대 폭 조정 |
...
| 개별 화면 | 목적지·MY·계정·확장 목적지의 네이티브 검증은 매트릭스 순서대로 후속 적용 |
...
| iPhone SE 3세대 목적지 | G1·G2·P1 375 × 667 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-g1-se3-web.png`, `/tmp/weatheron-g2-se3-web.png`, `/tmp/weatheron-p1-se3-results-web.png` |
| iPhone 17e 목적지 | G1·G2·P1 390 × 844 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-g1-17e-web.png`, `/tmp/weatheron-g2-17e-web.png`, `/tmp/weatheron-p1-17e-web.png` |
| iPhone 17 Pro Max 목적지 | G1·G2·P1 440 × 956 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-g1-promax-web.png`, `/tmp/weatheron-g2-promax-web.png`, `/tmp/weatheron-p1-promax-web.png` |
| iPhone SE 3세대 MY·설정 | M1~M4 375 × 667 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-m1-se3-web.png` ~ `/tmp/weatheron-m4-se3-web.png` |
| iPhone 17e MY·설정 | M1~M4 390 × 844 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-m1-17e-web.png` ~ `/tmp/weatheron-m4-17e-web.png` |
| iPhone 17 Pro Max MY·설정 | M1~M4 440 × 956 웹 뷰포트 렌더링 부분 통과 | `/tmp/weatheron-m1-promax-web.png` ~ `/tmp/weatheron-m4-promax-web.png` |
...
목적지 화면은 In-app browser의 QA 전용 초기 route 번들로 G1·G2·P1을 직접 시작함. 375·390·440pt에서 가로 잘림, G2 반복 토글·요일 선택, P1 검색 결과와 고정 CTA 배치를 확인함. 검증 후 초기 route 분기는 소스에서 제거함. iOS 네이티브 빌드는 기존 Expo 57 전환 이후 Pods의 React Native/Yoga 버전 불일치로 `yoga/style/GridLine.h`를 찾지 못해 중단됐으므로 네이티브 통과로 기록하지 않음.

MY·설정 화면은 M1에서 M2·M3·M4로 실제 이동해 375·390·440pt 렌더링을 확인함. M2 고급 설정 열기, M3 온도 단위 선택, M4 권한 카드의 2버튼 행까지 확인했으며 검증 후 초기 route 분기는 소스에서 제거함. 네이티브 빌드 차단 사유가 해소되지 않아 웹 검증만 `부분`으로 기록함.
