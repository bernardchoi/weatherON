...
`docs/Project Wind/`는 출시 후 대규모 UI 업데이트를 위한 신규 디자인 시스템 연구 트랙이다.
현행 MVP, Android 비공개 테스트, v1.0 출시 후보는 이 문서를 기준으로 유지한다.
Project Wind의 바람의 탑 기반 표면, 빛, 밀도, 모션 원칙은 별도 토큰/컴포넌트 검증을 거친 뒤 차기 메이저 UI 개편 후보로만 편입한다.
...
| 토큰 | 값 | 용도 |
|---|---|---|
| `NAVY` | `#17365D` | 앱 배경, 브랜드 필드 |
| `NAVY_DARK` | `#1F4E79` | 탭바, 강한 surface |
| `PANEL` | `#214A78` | 기본 카드 |
