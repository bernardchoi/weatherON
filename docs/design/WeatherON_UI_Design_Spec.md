...
- 강조 적용: iOS 빌드에서는 하단 탭바를 floating glass dock으로 강화하고, active chip, 뒤로가기·알림·검색 취소·목적지 전환 control, 바텀시트 handle/header에 더 낮은 투명도, 밝은 stroke, 얇은 상단 highlight를 적용해 Liquid Glass 존재감을 우선 노출한다. 출발 시간·이동수단처럼 즉시 조작하는 quick chip에도 같은 control 규칙을 적용한다.
- 금지: 홈 전체 배경, 핵심 CTA, 날씨·출발 카드 본문을 glass 표면으로 바꾸지 않는다.
...
- 적용 대상: 표시 설정의 segmented control/switch, 공통 목록, 하단 메뉴 내비게이션, 뒤로가기/아이콘 버튼, Sheet/Modal, Snackbar, 날씨 카드 일부, CTA 역할 구분.
- 표현 방식: `surfaceContainerLow` / `surfaceContainer` / `surfaceContainerHigh` / `secondaryContainer`, state layer/ripple, 선택 색상, Android elevation, bottom sheet drag handle 감각을 WeatherON 토큰으로 리틴트한다.
- Android 12 이상에서는 사용자가 `기기 색상 사용`을 직접 켤 수 있다. 기본값은 꺼짐이며 설정값을 로컬 DB에 저장한다.
- 기기 색상은 탭 active, segmented control 선택, switch, tonal button의 강조색에만 적용한다. 배경·날씨 기능색·핵심 CTA에는 적용하지 않는다.
- 상단 hero/header는 적용 대상에서 제외한다. 보조 레이어가 브랜드 헤더를 별도 부유 카드로 바꾸지 않게 한다.
- 하단 메뉴는 선택 배경 음영을 두지 않는다. 5px active dot과 아이콘·텍스트 강조색으로 상태를 구분하고, 누르는 순간에만 ripple을 표시한다.
...
이때 탭 개수, route, 아이콘 크기, active dot, safe area 기준은 위 표를 유지한다.
배경은 surface container와 Android elevation으로 조정한다. 선택 상태는 active dot과 아이콘·텍스트 색상으로 표시하고 ripple/state layer는 누르는 순간에만 사용한다.
...
- Android 공통 버튼은 `filled` / `tonal` / `outlined` / `text` 네 역할을 제공하고 48px 기본 높이와 pill shape를 사용한다. 작은 버튼도 40px 이상을 유지한다.
...
- 저장·삭제·복구 같은 짧은 결과 피드백은 화면 상단 카드 대신 하단 Snackbar를 사용한다.
- 기본 높이는 56px, radius는 10, surface는 inverse 계열, 액션 터치 영역은 48px 이상으로 둔다.
- 복구 액션이 있는 Snackbar는 3.8초 노출 후 종료한다. 아래로 44px 이상 쓸어내리면 즉시 닫히며, 출발 탭을 벗어나면 복구 가능 상태도 함께 폐기한다.
- 자동 종료와 스와이프 종료는 화면의 local visibility만 바꾸지 않고 복구 후보 상태까지 제거한다. 다른 탭을 거친 뒤 같은 Snackbar가 다시 나타나면 안 된다.
...
여행 확장 시 하단 탭 추가 없이 `출발 → 일상 이동 / 여행`으로 구성한다. 최초 진입은 일상 이동이며 예정 여행은 요약 카드로 연결한다. G3는 여행 목록·생성 진입, G4는 도보 코스, G5는 여러 날 도보 일정 안의 AI 계획으로 둔다. 도시 여행(국내·해외)과 도보 코스만 생성 유형으로 노출하며 AI를 세 번째 유형으로 나열하지 않는다. 무료 일정 추가·기본 공유는 허용하고, 저장 시 계정 연결·알림 신청 시 권한·유료 기능 선택 시 G6 구독을 각각 요청한다. 세부 한도는 `docs/planning/WeatherON_기능_출시_로드맵.md`의 여행 공통 기준을 따른다. 이 항목은 MVP 이후 설계이며 현재 구현 완료를 의미하지 않는다.
...
### G2 일상 이동 경로 확장 (2026-09-08 설계안)

- [일상 이동 경로 화면 기획](../planning/WeatherON_일상_이동_경로_화면_기획.md)을 상세 기준으로 사용한다. 구현 완료가 아니며 시각 토큰·플랫폼 스타일은 본 문서를 유지한다.
- G1은 3줄 목적지 요약 유지. G2는 기존 입력으로 출발·도착 시각, 이동 요약, 준비 안내까지 앱 안에서 판단하게 한다. 외부 지도 상세 경로 버튼은 보조 위계로 두고 새 경로 선택을 필수 단계로 추가하지 않는다.
- 시간 요약을 누르면 같은 시트에서 `도착 시간/출발 시간` 기준과 시각을 선택한다. 도착 기준은 출발 권장을 역산하고, 출발 기준은 선택 시각을 유지하며 경로 시간이 있을 때 예상 도착을 표시한다. 대중교통 선택 후에는 현재 계약의 예상시간만 갱신한다. 기존 카카오맵 REST 국내 상세 응답 계약을 확장한 뒤 다른 경로를 선택 시트에서 최대 3개 비교하며 버스·지하철·혼합 필터는 필요할 때 펼친다. Google 경로 API는 해외에만 사용한다.
- 이동 날씨는 G2의 자세히 펼침으로 제공한다. 별도 전체 화면·지도·길찾기 버튼 중복은 추가하지 않는다. 계산 근거는 접되 미확인·만료·오류 등 판단에 필요한 상태는 기본 화면에 남긴다.
- 초기 외부 연결 단계에 가상 경로·필터를 표시하지 않는다. 기존 선택을 새 추천으로 조용히 교체하거나 미리보기만으로 저장·알림을 실행하지 않는다.
- 최초 추천을 포함한 현재 표시 경로 하나의 날씨를 조회한다. 후보는 전체 응답에 필터를 적용한 뒤 최대 3개 표시한다. 기본 입력 누락과 미리보기/실제 예약 불일치는 같은 화면에 안내한다.
- 로딩·빈 결과·실패·큰 글씨·스크린리더·시트 복귀를 검증한다. 기존 입력이 있으면 추가 선택 없이 핵심 판단을 얻는지 목업에서 먼저 확인한다.

### P1 목적지 추가

- 검색 의도와 실제 장소 결과를 구분한다. 입력어와 겹치는 장소명 1~3개는 `연관 검색`으로 먼저 제시하고, 탭하면 그 문자열로 재검색한다.
- 실제 장소 행은 장소명 일치어를 Sky로 강조하고 주소를 먼저 보여준다. 다음 줄에는 현재·설정 위치 거리, 카테고리, 결과 성격만 짧게 표시한다.
- 정렬은 현재 위치가 있으면 거리 우선, 거리가 같을 때 검색어 일치 우선으로 유지한다. 제안 영역이 실제 장소 결과의 거리 정렬을 바꾸면 안 된다.
...
| 날짜 | 내용 |
|---|---|
| 2026-07-20 (2차) | P1 목적지 검색에 연관 검색, 일치어 강조, 주소→거리·카테고리 정보 순서를 적용. G1 삭제 복구 Snackbar는 3.8초로 단축하고 아래 스와이프·탭 이탈 시 즉시 폐기해 재진입 재노출을 방지 |
| 2026-07-21 | iOS Liquid Glass control 범위 강화. 하단 탭바를 floating dock으로 조정하고 뒤로가기·알림·검색 취소·목적지 전환·출발 quick chip·바텀시트 chrome을 공통 control 계층으로 정리. 날씨·출발 카드 본문은 기존 Clear Navy/Warm Sun 체계를 유지 |
