...
- 표현 방식: `surfaceContainerLow` / `surfaceContainer` / `surfaceContainerHigh` / `secondaryContainer`, state layer/ripple, active indicator, Android elevation, bottom sheet drag handle 감각을 WeatherON 토큰으로 리틴트한다.
...
- 하단 메뉴 active indicator는 64×32px로 아이콘 뒤에 고정하고 기존 5px active dot을 함께 유지한다. 비선택 항목과 구분되는 충분한 tonal 대비를 확보한다.
