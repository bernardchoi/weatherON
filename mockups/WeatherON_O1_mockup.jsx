...
/* ── WeatherON O1 · 스플래시 (하이브리드 크롬) ─────────────────────────
   와이어프레임 O1 기준: 앱 로딩 화면 — 브랜드 모먼트
   - 로고마크 + 워드마크 + 한 줄 태그라인("오늘의 외출, 미리 준비하세요")
   - 선택 온보딩 시작 시 사용 / 기본 신규 진입은 A1 스플래시 → H1 홈(Guest) 직행
   - 탭바 없음, 인터랙션 없음(순수 로딩 모먼트)
...
function Wordmark({ size = 24 }) {
  return (
    <div style={{ display:'flex', alignItems:'center', fontFamily:"'Manrope',sans-serif" }}>
      <span style={{ fontWeight:300, fontSize:size, color: INK(0.94), letterSpacing:'-0.4px' }}>weather</span>
...
export default function WeatherON_O1({ navigate, routeState = {} } = {}) {
  const weatherTheme = applyWeatherONTheme(routeState?.themeMode);
  useEffect(() => {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "https://fonts.googleapis.com/css2?family=Manrope:wght@400;700;800&family=Noto+Sans+KR:wght@400;500;700&family=DM+Mono:wght@400;500&display=swap";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: "radial-gradient(ellipse, rgba(240,160,32,0.18) 0%, transparent 70%)",
          pointerEvents: "none",
        }}/>
        ...
        <div style={{
          ...
          <AppIconGlyph/>
          <Wordmark size={24}/>
          <div style={{ fontSize: 13.5, color: INK(0.78), fontFamily: "'Noto Sans KR',sans-serif" }}>
            오늘의 외출, 미리 준비하세요
          </div>
        </div>
        ...
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 64, display: "flex", justifyContent: "center" }}>
          <div style={{ display: "flex", gap: 6 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: GOLD, opacity: i === 1 ? 1 : 0.35 }}/>
            ))}
          </div>
        </div>

        <div style={{ position: "absolute", left: 24, right: 24, bottom: 102, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <button onClick={() => navigate?.("O2")} style={{
            height: 44,
            borderRadius: 16,
            border: "none",
            background: GOLD,
          ...
          <button onClick={() => navigate?.("H1")} style={{
            height: 44,
            borderRadius: 16,
