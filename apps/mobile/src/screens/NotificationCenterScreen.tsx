...
type InboxItem = { id: string; title: string; route?: P0RouteId; observedAt: string; receivedAt?: string; openedAt?: string; readAt?: string };

export function NotificationCenterScreen({
  state, savedDestinations, readNotificationIds, notificationHistory, smartCareEnabled, permissionReady, notificationDeliveryStatus,
  ...
  const scheduleDetail = smartCareEnabled && permissionReady && notificationDeliveryStatus.status === "scheduled" ? "기기 예약 확인됨" : "예약 대상 계산됨";
  ...
  return <View style={[styles.shell, { backgroundColor: theme.background }]}>
    <ScrollView contentContainerStyle={[styles.content, { maxWidth: layout.contentMaxWidth, paddingHorizontal: layout.screenHorizontalPadding, paddingTop: layout.weatherTopPadding }]} showsVerticalScrollIndicator={false}>
      <View style={[styles.header, pageStyles.header]}>
        ...
        <View style={styles.headerCopy}><Text style={[styles.title, pageStyles.title, { color: theme.text }]}>알림 센터</Text><Text style={[styles.subtitle, pageStyles.compactCaption, { color: theme.subtle }]}>{unread.length ? "미확인 " + unread.length + "개" : inbox.length ? "모두 확인함" : "새 알림 없음"}</Text></View>
      ...
      {unread.length ? <View style={styles.listHead}><Text style={[styles.listTitle, pageStyles.sectionTitle, { color: theme.text }]}>새로 확인할 알림</Text><Pressable accessibilityLabel="전체 읽음 처리" accessibilityRole="button" onPress={onMarkAllNotificationsRead} hitSlop={8}><Text style={[styles.markAll, { color: theme.sky }]}>전체 읽음</Text></Pressable></View> : null}
      ...
      {scheduled.length ? <CompactSection theme={theme} title="예약된 알림" detail={scheduleDetail}>{scheduled.slice(0, 3).map((item) => <Text key={item.id} style={[styles.compactRow, { color: theme.muted }]} numberOfLines={2}>{item.title + " · " + formatDate(item.scheduledAt)}</Text>)}</CompactSection> : null}
      {(state.officialSpecialAlert.active || conditions.length > 0) ? <CompactSection theme={theme} title={state.officialSpecialAlert.active ? "현재 기상 특보" : "현재 조건"} detail={state.officialSpecialAlert.active ? "공식 발표" : "알림 조건 감지"}>{<Text style={[styles.compactRow, { color: theme.muted }]}>{state.officialSpecialAlert.active ? (state.officialSpecialAlert.title ?? "기상 특보") + " · " + (state.officialSpecialAlert.reason ?? "발표 내용 확인 필요") : conditions.map((item) => item.conditionSummary ?? item.reason).join(" · ")}</Text>}</CompactSection> : null}
      {notificationHistory.length ? <Pressable accessibilityLabel="알림 기록 삭제" accessibilityRole="button" onPress={onClearNotificationHistory} hitSlop={8} style={styles.clearHistory}><Text style={[styles.clearHistoryText, { color: theme.subtle }]}>알림 기록 삭제</Text></Pressable> : null}
    </ScrollView>
  </View>;
}
...
function NotificationRow({ item, read, targetExists, theme, onOpen, onRead }: { item: InboxItem; read: boolean; targetExists: boolean; theme: AppTheme; onOpen: () => void; onRead: () => void }) {
  ...
  const target = item.route ? targetExists ? getTargetLabel(item.route) : "목적지 목록" : "이동 대상 없음";
...
function formatLog(item: InboxItem) { return [["수신", item.receivedAt], ["열기", item.openedAt], ["읽음", item.readAt]].filter(([, value]) => value).map(([label, value]) => label + " " + formatTime(value)).join(" · "); }
...
function formatTime(value?: string) { const time = value ? Date.parse(value) : Number.NaN; return Number.isFinite(time) ? new Intl.DateTimeFormat("ko-KR", { hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(new Date(time)) : "시각 미상"; }
...
const styles = StyleSheet.create({
  ...
  listHead: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: spacing.xs }, listTitle: { fontSize: 17 }, markAll: { minHeight: 36, paddingHorizontal: spacing.xs, textAlignVertical: "center", fontSize: 13, fontWeight: "700" }, inbox: { gap: spacing.lg }, dateGroup: { gap: spacing.sm }, dateLabel: { fontSize: 13, fontWeight: "700", marginLeft: spacing.xs },
  row: { minHeight: 94, flexDirection: "row", gap: spacing.sm, borderWidth: 1, borderRadius: radius.lg, padding: spacing.md }, iconBox: { width: 40, height: 40, borderRadius: radius.md, alignItems: "center", justifyContent: "center" }, icon: { width: 21, height: 21 }, rowCopy: { flex: 1, minWidth: 0, gap: 2 }, rowMain: { flex: 1, gap: 2, paddingRight: 42 }, titleLine: { flexDirection: "row", alignItems: "flex-start", gap: 7 }, unreadDot: { width: 7, height: 7, borderRadius: radius.pill, marginTop: 6, flexShrink: 0 }, rowTitle: { flex: 1, fontSize: 16, lineHeight: 22, fontWeight: "700" }, rowMeta: { fontSize: 13, lineHeight: 19 }, pastLabel: { fontSize: 12, lineHeight: 17 }, rowActions: { position: "absolute", right: 0, top: 0, alignItems: "flex-end", gap: spacing.xs }, openHint: { minHeight: 28, fontSize: 13, fontWeight: "700", textAlignVertical: "center" }, readAction: { minHeight: 28, fontSize: 12, textAlignVertical: "center" }, eventLog: { fontSize: 11, lineHeight: 16, marginTop: 3 },
