...
function dateLabel(value?: string) { const date = value ? new Date(value) : null; if (!date || Number.isNaN(date.getTime())) return translateText("날짜 미상"); const today = new Date(); const yesterday = new Date(today); yesterday.setDate(today.getDate() - 1); if (date.toDateString() === today.toDateString()) return translateText("오늘"); if (date.toDateString() === yesterday.toDateString()) return translateText("어제"); return formatDisplayDate(date, { month: "long", day: "numeric", weekday: "short" }); }
function getPresentation(item: InboxItem, theme: AppTheme) { const kind = getNotificationKind(item.id); if (kind === "alert") return { icon: "uv" as const, color: theme.alert }; if (kind === "rain" || item.route === "H5") return { icon: "rain" as const, color: theme.sky }; if (item.route === "G2" || item.route === "H7") return { icon: item.route === "G2" ? "depart" as const : "clock" as const, color: theme.gold }; return { icon: "cloud" as const, color: theme.clear }; }
function getNotificationKind(id: string) { if (/^(heatwave|heavy-rain)/u.test(id)) return "alert"; if (/^(rain|umbrella)/u.test(id)) return "rain"; return "general"; }
function formatMeta(route: P0RouteId | undefined, target: string, timestamp?: string) { const time = formatDate(timestamp); return !route || route === "H1" ? time : target + " · " + time; }
function formatDate(value?: string) { const time = value ? Date.parse(value) : Number.NaN; return Number.isFinite(time) ? formatDisplayDate(time, { month: "2-digit", day: "2-digit", hour: "numeric", minute: "2-digit" }) : translateText("시각 미상"); }
